...
func TestGetLaunchCommand(t *testing.T) {
	...
	wantPrefix := []string{"grok", "--no-auto-update", "--permission-mode", "bypassPermissions", "--rules", "ao standing instructions", "-p", "do the thing"}
...
func TestGetLaunchCommandSystemPromptFromFile(t *testing.T) {
	...
	want := []string{"grok", "--no-auto-update", "--rules", "file standing instructions", "-p", "fix it"}
	if !reflect.DeepEqual(cmd, want) {
		t.Fatalf("cmd = %#v, want %#v", cmd, want)
	}
}
